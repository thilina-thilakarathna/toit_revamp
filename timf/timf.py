...
class TIMF:    
    ...
    def trust_assessment_baseline_ifum(self, provider_id, microcell_id):
        local_data = self.data_service.get_local_data(microcell_id, provider_id)
        df_dir,time_dir = self.tda_baseline.run_isolation_forest(local_data)
        return df_dir,time_dir
    
    def trust_assessment_baseline_lof(self, provider_id, microcell_id):
        local_data = self.data_service.get_local_data(microcell_id, provider_id)
        df_dir,time_dir = self.tda_baseline.run_lof(local_data)
        return df_dir,time_dir
    

    def trust_assessment_baseline_ocsvm(self, provider_id, microcell_id):
        local_data = self.data_service.get_local_data(microcell_id, provider_id)
        df_dir,time_dir = self.tda_baseline.run_ocsvm(local_data)
        return df_dir,time_dir
